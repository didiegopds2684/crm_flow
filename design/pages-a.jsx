...
const OverviewPage = () => (
  <div style={{ padding: 28, display:"flex", flexDirection:"column", gap: 22, overflow:"auto" }}>
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap: 14 }}>
      <StatCard label="Tenants ativos" value="12" delta="+2" sub="este mês"/>
      <StatCard label="Usuários" value="148" delta="+14" sub="7 pendentes de convite"/>
      <StatCard label="Entidades" value="36" sub="distribuídas entre os tenants"/>
      <StatCard label="Registros hoje" value="1.284" delta="+8%" sub="vs. ontem"/>
    </div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1.6fr 1fr", gap: 14 }}>
      <Card style={{ padding: 0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"16px 18px", borderBottom:"1px solid var(--line)" }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600 }}>Atividade por módulo</div>
      ...
      <Card>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 600 }}>Tenants em foco</div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: 14 }}>
      <Card>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>Eventos recentes</div>
      ...
      <Card>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>Próximos passos</div>
        <div style={{ display:"flex", flexDirection:"column", gap: 10 }}>
          {[
...
const TenantsPage = () => (
  <div style={{ padding: 28, display:"flex", flexDirection:"column", gap: 18, overflow:"auto" }}>
    <div style={{ display:"flex", alignItems:"center", gap: 10, flexWrap:"wrap" }}>
      <FilterChip label="Todos" count={TENANTS.length} active/>
      <FilterChip label="Ativos" count={4}/>
      <FilterChip label="Trial" count={1}/>
      <FilterChip label="Pausados" count={1}/>
      <div style={{ flex: 1 }}/>
      <SearchInput placeholder="Buscar por nome ou slug"/>
      <GhostBtn>Exportar CSV</GhostBtn>
      <GhostBtn>Filtros ▾</GhostBtn>
      <PrimaryBtn>+ Novo tenant</PrimaryBtn>
    </div>
...
function FilterChip({ label, count, active }) {
  return (
    <button style={{
      ...
      border: active ? "1px solid #13212f" : "1px solid var(--line)",
      background: active ? "#13212f" : "rgba(255,255,255,0.8)",
...
const UsersPage = () => (
  <div style={{ padding: 28, display:"flex", flexDirection:"column", gap: 18, overflow:"auto" }}>
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap: 14 }}>
      <StatCard label="Total" value="148"/>
      <StatCard label="Ativos" value="132" sub="89% da base"/>
      <StatCard label="Convites" value="11" delta="+3" sub="pendentes há >3d"/>
      <StatCard label="Suspensos" value="5"/>
    </div>

    <div style={{ display:"flex", alignItems:"center", gap: 10, flexWrap:"wrap" }}>
      <FilterChip label="Todos" count={148} active/>
      <FilterChip label="Admins" count={9}/>
      <FilterChip label="Managers" count={24}/>
      <FilterChip label="Operators" count={78}/>
      <FilterChip label="Viewers" count={37}/>
      <div style={{ flex: 1 }}/>
      <SearchInput placeholder="Nome, email ou tenant"/>
      <GhostBtn>Importar</GhostBtn>
      <AccentBtn>+ Convidar usuário</AccentBtn>
    </div>
