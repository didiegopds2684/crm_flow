...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const path = queryString
    ? `/api/v1/entities/${context.params.slug}/records?${queryString}`
    : `/api/v1/entities/${context.params.slug}/records`;

  let response: Response;
  try {
    response = await authorizedBackendFetch(path, {
      headers: {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const { tenantId: _tenantId, ...recordPayload } = body;
